...
function Brewerlist() {
  ...
  const filterValues = (array: any) =>
    array.filter((v: any, i: any) => breweries.indexOf(v) === i);
  ...
  const fetchDataAnother50 = async () => {
    try {
      ...
      setBreweries((prevState) => [...prevState, ...res.data]);
    } catch (error) {
      console.log(error);
    }
  };
        ...
        {breweries.map((item: Breweries) => (
          <div className="item" key={item.id}>
            <ul>
              ...
              <li>
                <span className="title">Brewery type:</span>
                {item.brewery_type || "N/A"}
              </li>
              <li>
      ...
      <div className="container">
        <div className="selectContainer">
          ...
          >
            ...
            <option data-testid="type-option" value="state">
              State
            </option>
            <option data-testid="type-option" value="brewery_type">
              Brewery Type
            </option>
          </select>
          <label htmlFor="sortBy">
            Search by {sortType === "state" ? "State: " : "Brewery Type: "}{" "}
          </label>
          <input
            data-testid="search"
            disabled={sortType ? false : true}
            name="sortby"
            type="text"
            onFocus={hideError}
          ...
          <button
            disabled={sortType ? false : true}
            type="submit"
            onClick={() => fetchByType(sortType, sortValue)}
          >
            Sort
          </button>
          <button type="submit" onClick={refreshResults}>
            Refresh results
          </button>
          {sortValueEmpty ? (
            <span className="error"> Please add a value</span>
          ) : null}
        </div>
        {mapBreweries()}
        {/* <Pagination
